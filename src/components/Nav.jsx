...
const Nav = () => {
  ...
  // Debounced search — uses GNews "search" endpoint with q param
  const handlesearch = (e) => {
    const searchvalue = e.target.value.trim();
    if (!searchvalue) return;
    clearTimeout(timer.current);
    timer.current = setTimeout(async () => {
      const res = await fetchdata("search", { q: searchvalue });
      setdata(res);
    }, 500);
  };
      ...
      <div className="py-4 px-8 shadow-2xl">
        ...
        <div className="flex items-center justify-between px-2 md:px-8">
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            <span>
              <IoIosHelpCircleOutline className="w-5 h-5 cursor-pointer hidden md:block" />
            </span>
            <span>
              <IoSettingsSharp className="w-5 h-5 cursor-pointer hidden md:block" />
            </span>
          </div>
